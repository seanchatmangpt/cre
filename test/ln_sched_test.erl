...
%% Record definitions (matching ln_sched)
-record(sched_choice, {
    type :: atom(),
    value :: term(),
    metadata :: term(),
...
init_replay_test() ->
    Choices = [#sched_choice{type = select_branch, value = a, metadata = {}, timestamp = 0}],
...
record_choice_nondet_test() ->
    State = ln_sched:init(nondeterministic),
    NewState = ln_sched:record_choice(select_branch, branch_a, State),
...
verify_choice_match_test() ->
    Choice = #sched_choice{type = select_branch, value = a, metadata = {}, timestamp = 0},
    ?assertEqual({ok, a}, ln_sched:verify_choice(select_branch, [a, b, c], Choice)).
...
verify_choice_not_available_test() ->
    Choice = #sched_choice{type = select_branch, value = d, metadata = {}, timestamp = 0},
    ?assertEqual({error, not_available}, ln_sched:verify_choice(select_branch, [a, b, c], Choice)).

%% @doc Test replay mode consumes choices.
replay_consumes_choice_test() ->
    Choices = [#sched_choice{type = task_selection, value = a, metadata = {}, timestamp = 0},
                #sched_choice{type = task_selection, value = b, metadata = {}, timestamp = 0}],
    State = ln_sched:init(replay, Choices),
    {Selected, State1} = ln_sched:choose([{a, 1}, {b, 2}, {c, 3}], State),
    ?assertEqual({a, 1}, Selected),
    ?assertEqual(1, State1#sched_state.position).
...
replay_value_not_available_test() ->
    Choices = [#sched_choice{type = task_selection, value = d, metadata = {}, timestamp = 0}],
    State = ln_sched:init(replay, Choices),
...
multiple_choices_nondet_test() ->
    State = ln_sched:init(nondeterministic),
    State1 = ln_sched:record_choice(select_branch, branch_a, State),
...
log_reversed_test() ->
    State = ln_sched:init(nondeterministic),
    State1 = ln_sched:record_choice(select_branch, first, State),
    State2 = ln_sched:record_choice(defer_race, second, State1),
    Log = ln_sched:get_log(State2),
    [First, Second] = Log,
    ?assertEqual(defer_race, First#sched_choice.type),
    ?assertEqual(select_branch, Second#sched_choice.type).
