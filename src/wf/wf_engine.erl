...
set_worklet(Engine, Worklet) ->
    gen_server:call(Engine, {set_worklet, Worklet}).

%%--------------------------------------------------------------------
%% @doc Applies worklet rules to check if a task should be replaced.
%%
...
%% @param Data The case data to evaluate conditions against
%% @param Now Current timestamp (for logging purposes)
%% @return {ok, ReplacementWorkflow} | {error, no_matching_rule}
%%
%% @end
%%--------------------------------------------------------------------
...
apply_worklet(Worklet, Task, Data, _Now) ->
    wf_worklet:select(Worklet, Task, Data).
...
update_workitem_status(#wf_case{work_items = WIs} = Case, WiId, Status, User, Now, State) ->
    ...
    {Case1, State#engine_state{cases = Cases}}.
