...
This module implements the Interleaved Routing pattern as a gen_yawl behaviour.
...
```erlang
> interleaved_routing:preset(t_distribute).
[p_start]
```

```erlang
> interleaved_routing:preset(t_pick_next).
[p_branch_pool,p_next_branch]
```

```erlang
> interleaved_routing:preset(unknown).
[]
```
